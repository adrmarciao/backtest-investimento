...
@Document(collection = "annual_indicators")
@CompoundIndex(name = "ticker_ano_idx", def = "{'ticker': 1, 'ano': 1}", unique = true)
public class AnnualIndicatorsDocument {

    @Id
    private String id; // composite: TICKER_ANO
    ...
    public AnnualIndicatorsDocument() {
    }

    public AnnualIndicatorsDocument(String ticker, int ano, BigDecimal pl, BigDecimal pvp, BigDecimal dividaEbitda, BigDecimal roe, BigDecimal dpa, BigDecimal lpa, BigDecimal vpa) {
        this.id = ticker != null ? ticker.trim().toUpperCase() + "_" + ano : null;
